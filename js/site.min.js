$(document).ready(function() {
	var h = new Audio("alert.mp3");
	var hh = new Audio("ding.mp3");
	var m = 0;
	var d;
	var e;
	var a;
	...
	var j = function(n) {
		$("body").removeClass("working").addClass(n);
	};

	function everyFive(periods) { 
		window.parent.document.title = periods[5] + ':' + periods[6] + "  保持专注";
		if(periods[5] == 0 && periods[6] == 0)
			window.parent.document.title = "Here we go!一小时专注";
      //console.log(periods[5] + ':' + periods[6];
	...
	if (typeof(Storage) !== undefined) {
		for (var l in localStorage) {
			if (l == "count" || l.indexOf("task") === -1) {
				continue;
			}
			$("#done").prepend(localStorage.getItem(l));
		}
		if (localStorage.count) {
			$(".count").text(localStorage.count);
			i = localStorage.count;
			$("body").addClass("list");
		} else {
			$("body").addClass("new");
		}
	...
	$("#cycle").on("input", function(n) {
		this.value = this.value.replace(/[^0-9]/g, "");
		g = $(this).val();
	});
	$("#nextCycle").on("input", function(n) {
		this.value = this.value.replace(/[^0-9]/g, "");
		e = $(this).val();
	});
	$("#cycle-form").submit(function() {
		...
		if ($("#cycle").val() === "") {
			...
			localStorage.setItem("cycle", g);
			localStorage.setItem("nextCycle", g);
		}
		d = localStorage.cycle;
		$("body").removeClass("settings-open");
		return false;
	});
	$("#next-form").submit(function() {
		localStorage.setItem("nextCycle", e);
		e = localStorage.nextCycle;
		$("body").removeClass("settings-open");
		return false;
	});
	...
	$("#end").on("click", function() {
		m = $("#timer span").countdown("getTimes");
		$("#timer span").countdown("option", {
			until: 0,
			onExpiry: f
		});
	...
	$("#logActivity").submit(function() {
		...
		var o = $("#what").val();
		var s = new Date();
		var n = ("0" + s.getHours()).slice(-2);
		var q = ("0" + s.getMinutes()).slice(-2);
		var p;
		i++;
		localStorage.count = i;
		h.currentTime = 0;
		h.pause();
		if (typeof m[5] !== "number") {
			p = 0;
		} else {
			p = m[5];
		}
		var r = "<li data-task='task" + ("0" + i).slice(-2) + "'><h6>" + b + ":" + c + " - " + n + ":" + q + "</h6><br /><p>" + o + "<small>" + (g - p) + " 分钟</p></li>";
		if (e !== g) {
			g = localStorage.nextCycle;
			localStorage.setItem("cycle", localStorage.nextCycle);
		}
		localStorage.setItem("task" + ("0" + i).slice(-2), r);
		$("#done").prepend(r);
		if ($("#new").is(":checked")) {
			$("#timer span").countdown("option", {
				until: +g * 60
	...
	$("#done").on("mouseenter", "li[data-task]", function() {
		...
		$(this).find(".delete").on("click", function() {
			var n = $(this).parent().parent().attr("data-task");
